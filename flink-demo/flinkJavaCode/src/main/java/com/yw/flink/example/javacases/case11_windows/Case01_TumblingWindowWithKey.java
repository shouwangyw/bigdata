 ...
 */
public class Case01_TumblingWindowWithKey {
        ...
        keyedStream.window(TumblingEventTimeWindows.of(Time.seconds(5)))
                .process(new ProcessWindowFunction<StationLog, String, String, TimeWindow>() {
                    ...
                    public void process(String key,
                        ...
                        //统计当前基站对应的通话时长
                        long totalDurtionTime = 0;
                        for (StationLog element : elements) {
                            totalDurtionTime += element.duration;
                        }

                        //返回数据
                        collector.collect("窗口范围：[" + startTime + "~" + endTime + "),基站ID:" + key + ",通话总时长：" + totalDurtionTime);
                    }
                }).print();
        env.execute();
    }

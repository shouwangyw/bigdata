 ...
 */
public class Case04_GlobalWindowWithoutKey {
    public static void main(String[] args) throws Exception {
        ...
        dsWithWatermark.windowAll(GlobalWindows.create())
                ...
                .trigger(new MyGlobalCountTrigger()).process(new ProcessAllWindowFunction<StationLog, String, GlobalWindow>() {
                    @Override
                    public void process(ProcessAllWindowFunction<StationLog, String, GlobalWindow>.Context context, Iterable<StationLog> elements, Collector<String> collector) throws Exception {
                        //统计通话时长
                        long totalDurationTime = 0L;
                        for (StationLog element : elements) {
                            totalDurationTime += element.duration;
                        }
                        collector.collect("全局窗口触发，最近3条数据的通话时长：" + totalDurationTime);
                    }
                }).print();
        env.execute();
     ...
     */
    private static class MyGlobalCountTrigger extends Trigger<StationLog, GlobalWindow> {
        //定义状态描述器

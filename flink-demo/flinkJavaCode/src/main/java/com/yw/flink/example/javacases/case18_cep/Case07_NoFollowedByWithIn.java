...
/**
 * Flink CEP : NotFollowedBy + Within
 */
public class Case07_NoFollowedByWithIn {
    public static void main(String[] args) throws Exception {
        ...
        Pattern<String, String> pattern = Pattern.<String>begin("first")
                ...
                }).notFollowedBy("second").where(new SimpleCondition<String>() {
                    @Override
                    public boolean filter(String s) throws Exception {
                        return s.startsWith("c");
                    }
                }).followedBy("third").where(new SimpleCondition<String>() {
                    @Override
                    public boolean filter(String s) throws Exception {
        ...
        SingleOutputStreamOperator<String> result = patternStream
                .select((PatternSelectFunction<String, String>) map -> {
                    ...
                    String third = map.get("third").get(0);
                    return first + "-" + third;
                });

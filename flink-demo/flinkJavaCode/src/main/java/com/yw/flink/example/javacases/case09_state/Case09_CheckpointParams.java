...
public class Case09_CheckpointParams {
    public static void main(String[] args) {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        // 开启checkpoint，每隔1000ms往数据源插入一个barrier，进行一次checkpoint
        env.enableCheckpointing(1000);

        // 设置 checkpoint storage存储为JobManagerStorage，默认堆内存存储状态大小为5M
        env.getCheckpointConfig().setCheckpointStorage(new JobManagerCheckpointStorage(5 * 1024 * 1024));

        // 设置 checkpoint storage存储为hdfs路径
        ...
        // 设置可容忍 checkpoint 失败次数，没有默认值值，表示不容忍任何checkpoint失败
        ...
        // 设置 checkpoint barrier 不对齐机制，设置此值时，checkpoint mode必须为EXACTLY_ONCE且MaxConcurrentCheckpoints为1

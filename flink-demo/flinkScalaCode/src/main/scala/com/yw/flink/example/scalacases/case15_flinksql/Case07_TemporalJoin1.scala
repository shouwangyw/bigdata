...
/**
  * Flink SQL - 使用时态表函数方式查询时态表中数据
  * 案例：读取Kafka topic数据形成主表和时态表，从时态表中查询时态数据
  */
object Case07_TemporalJoin1 {
  def main(args: Array[String]): Unit = {
    ...
    //创建时态表函数
    val temporalTableFunction = tableEnv.from("product_tbl")
      .createTemporalTableFunction($"right_rowtime", $"right_product_id")

    //注册时态表函数
    tableEnv.createTemporarySystemFunction("temporalTableFunction", temporalTableFunction)

    //        tableEnv.executeSql("select * from product_tbl").print();
    ...
    val result = tableEnv.sqlQuery("" +
      ...
      "   right_product_name," +
      "   right_price " +
      "from visit_tbl ,LATERAL TABLE (temporalTableFunction(left_rowtime)) " +

...
import org.apache.flink.table.api.{AnyWithOperations, DataTypes, FieldExpression, Schema, Table}
...
/**
  * Flink Table 转换成DataStream - tableEnv.toChangelogStream(table,schema,changelogMode)
  * 1.创建DataStream
  * 2.设置watermark
  * 3.dataStream -> Table (传递watermark)
  * 4.toChangelogStream:table -> DataStream ： 观察是否有watermark传递
  */
object Case10_ToChangelogStream2 {
  def main(args: Array[String]): Unit = {
    ...
    //对table进行聚合
    val resultTbl: Table = table.groupBy($"sid")
      .select($"sid", $"duration".sum().as("total_duration"))

    //toChangelogStream:table -> DataStream ： 观察是否有watermark传递
    val result: DataStream[Row] = tableEnv.toChangelogStream(resultTbl, Schema.newBuilder()
      .column("sid", DataTypes.STRING())
      .column("total_duration", DataTypes.BIGINT())

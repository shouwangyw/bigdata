...
object TrainLRwithLBFGS {
  ...
  def main(args: Array[String]) {
    ...
    val temp = camera_ids.map({ camera_id =>
      ...
      /**
       * relations中存储了每一个卡口在day这一天每一分钟的平均速度
       */
      ...
      for (i <- Range(60 * hours, 0, -1)) {
        ...
        if (labels.toArray.length == 1) {
          ...
          val record =
          // if ((label.toInt/10)<10) (label.toInt/10) else 10.0
          // 计算结果0-10 11种可能？为了提高抗噪声能力
      ...
      if (!data.isEmpty()) {
        // 训练逻辑回归模型
        // LogisticRegressionWithLBFGS 可以训练出来一个具备多分类功能的模型
         ...
         */
        if (precision > 0.8) {
          val path = "hdfs://mycluster/model/model_" + camera_id + "_" + nowtimelong
          model.save(sc, path)

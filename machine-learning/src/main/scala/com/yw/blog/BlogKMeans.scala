...
object BlogKMeans {
  def main(args: Array[String]) {
    ...
    val rdd = sc.textFile("machine-learning/data/original.txt", 8)

    //    map mapPartition  HBase Redis ES      foreachPartition   foreach
    /**
     * mapPartitions: 将每一篇微博内容进行分词，使用IKAnalyzer分词器（过滤掉停用词）
     ...
     * 如果application是一个job的，cache 是没意义的
     * persist()
     */
...
//    tfIdfs.foreach(println)
    ...
    // kmeansModel.save(sc, "model001")
    // 打印模型的20个中心点
    println(kmeansModel.clusterCenters.mkString("Array(", ", ", ")"))
    ...
    val result: RDD[(String, (Int, (Vector, ArrayBuffer[String])))] =
      predictionRDD.join(tfIdfs2wordsRDD)

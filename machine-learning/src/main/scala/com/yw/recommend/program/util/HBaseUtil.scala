...
object HBaseUtil {
   ...
   * 通过RowKey获取某张表的数据
   */
  def getRecord(tableName: String, rowKey: String, conn: Connection): mutable.Seq[String] = {
  ...
  def getRecord1(tableName: String, rowKey: String): mutable.Seq[String] = {
    val conn = getConn(tableName)
    val table: Table = conn.getTable(TableName.valueOf(tableName))
    val get: Get = new Get(Bytes.toBytes(rowKey))
    val result: Result = table.get(get)
  ...
  def getRecord2(tableName: String, rowKey: String): mutable.Seq[String] = {
    val conn = getConn(tableName)
    val table: Table = conn.getTable(TableName.valueOf(tableName))
    val get: Get = new Get(Bytes.toBytes(rowKey))
    val result: Result = table.get(get)
  ...
  def getRecord3(tableName: String, rowKey: String, conn: Connection): mutable.Seq[String] = {
    val table: Table = conn.getTable(TableName.valueOf(tableName))
    val get: Get = new Get(Bytes.toBytes(rowKey))
    val result: Result = table.get(get)

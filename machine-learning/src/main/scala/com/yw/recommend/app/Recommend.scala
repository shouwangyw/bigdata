package com.yw.recommend.app

import org.apache.log4j.{Level, Logger}
import org.apache.spark.mllib.classification.{LogisticRegressionModel, LogisticRegressionWithSGD2}
...
object Recommend {
  def main(args: Array[String]) {
    ...
    // 加载数据，用\t分隔开
    val data: RDD[Array[String]] = sc.textFile("machine-learning/data/recommend/train_data/000000_0").map(_.split("\t"))
    ...
    val la: RDD[LabeledPoint] = label.map {
      case "-1" => 0.0
      case "1" => 1.0
...
//    println("done")
    ...
    // 创建模型实例
        val lr = new LogisticRegressionWithSGD2(1.0, 100, 0, 1.0)
...
//    LogisticRegressionModel.load()
    // 输出
    val pw = new PrintWriter("./machine-learning/data/model");
    // 遍历

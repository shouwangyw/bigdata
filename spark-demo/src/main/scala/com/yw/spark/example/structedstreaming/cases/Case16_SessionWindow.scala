...
/**
  * 读取Socket 数据设置会话窗口，如果数据为a 那么gap 为5s ,如果数据为b 那么gap 为10s ,其他的gap 为20s 。
  */
object Case16_SessionWindow {
  def main(args: Array[String]): Unit = {
    ...
    val transDF: DataFrame = tsAndWordsDF
      .groupBy(
        session_window($"timestamp", when($"word" === "a", "5 seconds")
          .when($"word" === "b", "10 seconds")

...
/**
  * Spark整合iceberg，合并iceberg表的数据文件
  * @author yangwei
  */
object Case04_SparkIcebergMergeFile {
...
//
//    /******************* 2. 向表my_test中插入一批数据 *******************/
//    val df: DataFrame = spark.read.textFile("/input/name_info.txt")
//      .map(line => {
...
//    df.writeTo("hadoop_prod.default.my_test").append()
    ...
    /******************* 3. 合并小文件数据 *******************/
    val catalog = new HadoopCatalog(new Configuration(), "hdfs://mycluster/spark_iceberg")
